...
void jogar(int ordemMatriz, char **matrizVisivel, int **matrizBomba) {
    ...
    do {
        ...
        do {
            ...
            coordenadaLinha--;  // Ajusta para o índice da matriz (começando em 0)
            coordenadaColuna--; // Ajusta para o índice da matriz (começando em 0)
    ...
    } while (verificaVitoriaDerrota(ordemMatriz, matrizVisivel, matrizBomba) != 0 && matrizBomba[coordenadaLinha][coordenadaColuna] != -1);

    if(matrizBomba[coordenadaLinha][coordenadaColuna] == -1){
        printf("it's over pro betinha, so lhe resta o churrascamento!\n");
    } 
    else {
...
void imprimeMatrizBomba(int ordemMatriz, int **matrizBomba){
    printf("Aqui esta a matriz original, -1 representa onde estao as bombas:\n");
    for(int contL = 0; contL < ordemMatriz; contL++) {
        printf("%02d| ", contL + 1); // Índice da linha
        for (int contC = 0; contC < ordemMatriz; contC++) {
